...
const FormModal: React.FC<FormModalProps> = ({ mode, initial, onClose, onSaved, editId }) => {
  const { t } = useTranslation('modelGroups');
  ...
  const saveLabel = saving === 'uploading' ? t('form.uploading')
                  : saving === 'saving'    ? t('form.saving')
                  : mode === 'add'         ? t('form.addTitle')
                  : t('form.saveChanges');
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 16px' }}>
            ...
            {/* Brand */}
            <div>
              <label style={LBL}>{t('form.brand')} <span style={{ color: '#ef4444' }}>*</span></label>
              <input type="text" required value={form.brand} placeholder={t('form.brandPlaceholder')}
                onChange={e => handleBrand(e.target.value)} onFocus={onFocus} onBlur={onBlur} style={F} />
            </div>

            {/* Model */}
            <div>
              <label style={LBL}>{t('form.model')} <span style={{ color: '#ef4444' }}>*</span></label>
              <input type="text" required value={form.model} placeholder={t('form.modelPlaceholder')}
            ...
            <div style={{ gridColumn: 'span 2' }}>
              <label style={LBL}>
                ...
                <span style={{ fontWeight: 400, color: '#9ca3af', marginInlineStart: 6 }}>{t('form.autoGenerated')}</span>
              </label>
              <input type="text" required value={form.name} placeholder={t('form.namePlaceholder')}
                onChange={e => handleName(e.target.value)} onFocus={onFocus} onBlur={onBlur} style={F} />
            </div>

            {/* Category */}
            <div>
              <label style={LBL}>{t('form.category')}</label>
              <select value={form.category} onChange={e => setField('category', e.target.value)}
                onFocus={onFocus} onBlur={onBlur} style={{ ...F, cursor: 'pointer' }}>
                {['Economy', 'Middle', 'Luxury', 'SUV', 'Van', 'Electric'].map(o => <option key={o} value={o}>{t(`category.${o}`)}</option>)}
              </select>
            </div>

            {/* Transmission */}
            <div>
              <label style={LBL}>{t('form.transmission')}</label>
              <select value={form.transmission} onChange={e => setField('transmission', e.target.value)}
                onFocus={onFocus} onBlur={onBlur} style={{ ...F, cursor: 'pointer' }}>
                {['Automatic', 'Manual'].map(o => <option key={o} value={o}>{t(`transmission.${o}`)}</option>)}
              </select>
            </div>

            {/* Fuel */}
            <div>
              <label style={LBL}>{t('form.fuel')}</label>
              <select value={form.fuel} onChange={e => setField('fuel', e.target.value)}
                onFocus={onFocus} onBlur={onBlur} style={{ ...F, cursor: 'pointer' }}>
                {['Petrol', 'Diesel', 'Hybrid', 'Electric', 'Other'].map(o => <option key={o} value={o}>{t(`fuel.${o}`)}</option>)}
              </select>
            </div>

            {/* Seats */}
            <div>
              <label style={LBL}>{t('form.seats')}</label>
            ...
            {/* Luggage */}
            <div>
              <label style={LBL}>{t('form.luggage')}</label>
              <input type="number" min="0" placeholder={t('form.luggagePlaceholder')}
            ...
            {/* Daily KM */}
            <div>
              <label style={LBL}>{t('form.dailyKm')}</label>
              <input type="number" min="0" placeholder={t('form.dailyKmPlaceholder')}
            ...
            {/* Monthly KM */}
            <div>
              <label style={LBL}>{t('form.monthlyKm')}</label>
              <input type="number" min="0" placeholder={t('form.monthlyKmPlaceholder')}
            ...
            {/* Deposit */}
            <div>
              <label style={LBL}>{t('form.deposit')}</label>
              <input type="number" min="0" step="0.01" placeholder={t('form.depositPlaceholder')}
            ...
            {/* Min Age */}
            <div>
              <label style={LBL}>{t('form.minAge')}</label>
              <input type="number" min="18" max="99" placeholder={t('form.minAgePlaceholder')}
            ...
            <div style={{ gridColumn: 'span 2' }}>
              ...
              <input type="number" min="0" step="0.01" required placeholder={t('form.pricePlaceholder')}
            ...
            <div style={{ gridColumn: 'span 2' }}>
              <label style={LBL}>{t('form.carImage')}</label>
              ...
              {previewUrl && (
                <div style={{ position: 'relative', marginBottom: 10 }}>
                  ...
                  <button type="button" onClick={() => { setImageFile(null); setPreviewUrl(null); setField('image_url', null); }}
                    style={{ position: 'absolute', top: 6, insetInlineEnd: 6, width: 24, height: 24, borderRadius: '50%', border: 'none', background: 'rgba(0,0,0,0.5)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
              ...
              >
                ...
                <span style={{ fontSize: 12, color: '#6b7280', fontWeight: 500 }}>
                  {previewUrl ? t('form.replaceImage') : t('form.uploadImage')}
                </span>
                <span style={{ fontSize: 11, color: '#9ca3af' }}>{t('form.formats')}</span>
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 7, minWidth: 0 }}>
        ...
        <div>
          ...
          <span style={{ fontSize: 11, color: '#9ca3af', marginInlineStart: 2 }}>{t('perDay')}</span>
        </div>
      </div>
...
const ModelGroupsPage: React.FC = () => {
  ...
  const handleSaved = () => {
    showToast(modal?.mode === 'add' ? t('toast.added') : t('toast.saved'), 'success');
    fetchGroups();
  };
  ...
  return (
    <div style={{ minHeight: '100%', background: 'linear-gradient(160deg, #f8fafc 0%, #f1f5f9 100%)', padding: '44px 40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 36 }}>
        <div>
          ...
          <p style={{ fontSize: 15, color: '#6b7280' }}>
            {loading ? t('loading') : t('count.groups', { count: groups.length })}
          </p>
        </div>
      </div>
      ...
      {/* Error */}
      {error && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#fff', border: '1px solid rgba(239,68,68,0.2)', borderInlineStart: '4px solid #ef4444', borderRadius: 12, padding: '14px 18px', marginBottom: 28, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#0f1117', marginBottom: 2 }}>{t('loadError')}</div>
      ...
      {!loading && !error && (['Economy', 'Middle', 'SUV'] as const).map(category => {
        ...
        return (
          <div key={t(`category.${category}`)} style={{ marginBottom: 28 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 12.5, color: '#c0c4cc' }}>{t('count.models', { count: section.length })}</span>
